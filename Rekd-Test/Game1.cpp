...
void Game1::Load()
{
	...
	m_Shader = new Shader(
		...
		"uniform float time; varying vec4 color; uniform sampler2D sampler; void main() { gl_FragColor = vec4(time - texture2D(sampler, gl_TexCoord[0].st).rgb * color.rgb, texture2D(sampler, gl_TexCoord[0].st).a); }"
		);
	m_Shader->Bind();
